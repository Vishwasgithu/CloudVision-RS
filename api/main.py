import os
os.environ["KMP_DUPLICATE_LIB_OK"] = "TRUE"  # fixes OMP Error #15 (duplicate libiomp5md.dll)
...
def pick_generator_checkpoints():
    ...
    baseline = ("outputs/checkpoints/gan/best_generator.pt", "RICE2 baseline")
    finetuned_candidates = [
        ("outputs/checkpoints/gan_liss4_g_only/best_generator_g_only.pt", "LISS-IV fine-tuned (generator-only)"),
        ("outputs/checkpoints/gan_liss4_finetune/best_generator_liss4.pt", "LISS-IV fine-tuned (adversarial)"),
    ]
    ...
    if not os.path.exists(baseline[0]):
        raise FileNotFoundError("No RICE2 baseline generator found -- this should always exist.")
...
gen_ckpt_baseline, gen_label_baseline, gen_ckpt_finetuned, gen_label_finetuned = pick_generator_checkpoints()

gen_model_rice2 = Generator(in_channels=gan_config['in_channels'], features=gan_config['features_g']).to(DEVICE)
...
if gen_ckpt_finetuned:
    gen_model_liss4 = Generator(in_channels=gan_config['in_channels'], features=gan_config['features_g']).to(DEVICE)
    gen_model_liss4.load_state_dict(load_state(gen_ckpt_finetuned))
    gen_model_liss4.eval()
    print(f"  -> LISS-IV fine-tuned: {gen_ckpt_finetuned} ({gen_label_finetuned}) (used for real GeoTIFF/AOI scenes)")
else:
    gen_model_liss4 = gen_model_rice2  # fall back to baseline until a fine-tuned checkpoint exists
    print("  -> No LISS-IV fine-tuned checkpoint found yet; real-scene mode will fall back to RICE2 baseline")

MODEL_INFO = {
    "segmentation_checkpoint": seg_ckpt_path, "segmentation_label": seg_label,
    ...
    "generator_liss4_label": f"{gen_label_finetuned} (used for real scenes)" if gen_ckpt_finetuned
...
seg_transform = A.Compose([A.Normalize(mean=(0,0,0), std=(1,1,1), max_pixel_value=255.0), ToTensorV2()])

def gaussian_weight(size=256):
    center = size // 2
    w = np.zeros((size, size)); w[center, center] = 1.0
...
def run_pipeline(img_rgb_u8, gen_model, threshold=0.5, mc_samples=5):
    ...
    for r in range(0, H_pad - PATCH_SIZE + 1, STRIDE):
        for c in range(0, W_pad - PATCH_SIZE + 1, STRIDE):
            ...
            edge_map = np.sqrt(sx**2 + sy**2); edge_map /= (edge_map.max() + 1e-8)

            img_norm = (patch_img.astype(np.float32) / 127.5) - 1.0
            x_input = np.concatenate([img_norm, patch_mask[:,:,None], edge_map[:,:,None]], axis=2)
            x_tensor = torch.from_numpy(x_input).permute(2,0,1).unsqueeze(0).to(DEVICE)
            ...
            for ch in range(3):
                output_accum[r:r+PATCH_SIZE, c:c+PATCH_SIZE, ch] += blended[:,:,ch] * GAUSS_W
...
def compute_real_metrics(clean_u8, label_u8):
    ...
    C1, C2 = 0.01**2, 0.03**2
    ssim = ((2*mu_p*mu_t+C1)*(2*spt+C2)) / ((mu_p**2+mu_t**2+C1)*(sp**2+st**2+C2))
    Rp, Gp, Bp = p[...,0], p[...,1], p[...,2]
    Rt, Gt, Bt = t[...,0], t[...,1], t[...,2]
    dp = np.clip(Gp+Rp-Bp, 0.1, None); dt = np.clip(Gt+Rt-Bt, 0.1, None)
    vari_p = np.clip((Gp-Rp)/dp, -1, 1); vari_t = np.clip((Gt-Rt)/dt, -1, 1)
    vari_rmse = float(np.sqrt(((vari_p - vari_t)**2).mean()))
    return {"psnr_db": round(float(psnr), 2), "ssim": round(float(ssim), 4), "vari_rmse": round(vari_rmse, 4)}
...
def compute_mask_accuracy(pred_mask_bin, true_mask_bin):
    ...
    dice = float(2 * intersection / (pred_mask_bin.sum() + true_mask_bin.sum())) \
        if (pred_mask_bin.sum() + true_mask_bin.sum()) > 0 else 1.0
    return {"iou": round(iou, 4), "dice": round(dice, 4)}

def compute_ndvi_stats(nir, red):
    ndvi = (nir.astype(np.float32) - red.astype(np.float32)) / np.clip(nir.astype(np.float32) + red.astype(np.float32), 1, None)
    return {"ndvi_mean": round(float(ndvi.mean()), 4), "ndvi_std": round(float(ndvi.std()), 4)}
...
@app.get("/api/samples")
def get_samples():
    manifest_path = Path("data/processed/patches/test/patch_manifest.json")
    ...
    for pid, meta in manifest.items():
        ...
        if coverage < 30:
            ...
            bin_label = "heavy"
        cloud_path = Path("data/processed/patches/test/cloud") / f"{pid}.png"
        label_path = Path("data/processed/patches/test/label") / f"{pid}.png"
        ...
        samples.append({
            "id": pid, "name": f"{pid}.png", "url": f"/static/uploads/{dest_name}",
            "cloud_coverage_pct": round(coverage, 1), "bin": bin_label,
            "has_ground_truth": label_path.exists()
        })
    # cap per-bin count so the sidebar stays scannable, but keep all three bins represented
    by_bin = {"light": [], "medium": [], "heavy": []}
    for s in samples:
        by_bin[s["bin"]].append(s)
    capped = by_bin["light"][:10] + by_bin["medium"][:10] + by_bin["heavy"][:10]
    return capped

@app.post("/api/process")
async def process_sample(file: UploadFile = File(None), sample_id: str = Form(None),
                          threshold: float = Form(0.5), mc_samples: int = Form(5)):
    ...
    if file:
        ...
        filepath = UPLOAD_DIR / filename
        import shutil
        shutil.copy(str(cloud_path), str(filepath))  # actually serve it, don't just read the original
        maybe_label = Path("data/processed/patches/test/label") / f"{sample_id}.png"
        if maybe_label.exists():
            label_path = maybe_label
        maybe_mask = Path("data/processed/patches/test/mask") / f"{sample_id}.png"
    ...
    mask, clean, unc_heatmap, coverage = run_pipeline(img, gen_model_rice2, threshold, mc_samples)

    mask_name, clean_name, unc_name = f"mask_{file_id}.png", f"clean_{file_id}.png", f"unc_{file_id}.png"
...
scene_registry = {}  # scene_id -> {path, band_files/internal_names, width, height, transform, crs}

def find_band_in_zip(zip_path, keyword):
    with zipfile.ZipFile(zip_path) as z:
        for name in z.namelist():
            if keyword.lower() in name.lower() and name.lower().endswith(('.tif', '.tiff')):
                return name
    return None
...
async def upload_scene(file: UploadFile = File(...)):
    ...
    if ext == ".zip":
        ...
        if not (g_name and r_name and nir_name):
            raise HTTPException(400, "Could not find BAND2/BAND3/BAND4 GeoTIFFs inside zip.")
        ...
        band_paths = {"green": str(save_path), "red": str(save_path), "nir": str(save_path)}
    else:
    ...
    scene_registry[scene_id] = {"band_paths": band_paths, "width": width, "height": height,
                                 "transform": transform, "crs": str(crs), "is_zip": ext == ".zip"}
    ...
    preview = np.dstack([norm_u8(n), norm_u8(r), norm_u8(g)]).astype(np.uint8)  # false color NIR-R-G
    preview_name = f"preview_{scene_id}.png"
    cv2.imwrite(str(SCENES_DIR / preview_name), cv2.cvtColor(preview, cv2.COLOR_RGB2BGR))

    return {
        "scene_id": scene_id, "width": width, "height": height,
        "preview_url": f"/static/scenes/{preview_name}",
        "preview_width": preview_w, "preview_height": preview_h,
        "max_aoi_size": MAX_AOI_SIZE,
    }

@app.post("/api/process_region")
async def process_region(scene_id: str = Form(...), x: int = Form(...), y: int = Form(...),
    ...
    mask, clean, unc_heatmap, coverage = run_pipeline(img_rgb, gen_model_liss4, threshold, mc_samples)
    ndvi_before = compute_ndvi_stats(n, r)

    file_id = str(uuid.uuid4())
    cloudy_name, mask_name, clean_name, unc_name = (
        f"aoi_cloudy_{file_id}.png", f"aoi_mask_{file_id}.png",
        f"aoi_clean_{file_id}.png", f"aoi_unc_{file_id}.png")
